...
void help(void)
{
	...
	puts("  -s <x,y,z...>  --set-correction    Sets correction to specific values");
	puts("  -t             --test-center       Tests if joystick is corectly calibrated");
	puts("                                       returns 1 if axes are wrong and 2 if buttons");
	puts("  -V             --version           Prints the version numbers");
	puts("  -p             --print-correction  Prints the current settings as a jscal command line");
	putchar('\n');
}
