 ...
 *****
 ***** Wanderlust.c 
 ***** modified wander.c with button input to allow facilitated demos.
 ***** Brian Rudy brudy@praecogito.com 11/6/2000
 ***************************************************************************
 ***************************************************************************
 ***************************************************************************/
...
int
buttonStatusCallback ( buttonStatusType *data ) {
  ...
  if (data->red_button_pressed) {
     ...
     /* demoShutDown(NULL); */
     disablePolling(demoModule);
  }
  if (data->yellow_button_pressed) {
     fprintf(stderr,"Yellow button pressed, starting demo...\n");
     createDemoModule();            
  }
  ...
  if (data->green_button_pressed) {
     fprintf(stderr,"Green button pressed\n");
  }
 
  /* Not quite sure what the difference is yet. Probably a latching input */
  if (data->red_button_changed) {
     fprintf(stderr,"Red button changed\n");
  }
  if (data->yellow_button_changed) {
     fprintf(stderr,"Yellow button changed\n");
  }
...
/*  demoModule=makeModule("Demo", NULL); */
  demoModule=makeModule("Demo", demoShutDown);
...
void demoShutDown(RaiModule* mod)
{
  disablePolling(mod);
}
...
main(int argc, char** argv )
{
  ...
  initClient("WanderLust", commShutdown); /* close function called if */
  ...
  registerButtonStatusCallback( buttonStatusCallback );
  ...
  /* set up your module to move robot*/
/*  createDemoModule();           */    
