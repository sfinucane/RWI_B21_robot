 ...
 *****
 ***** wander-laser 
 ***** Enhanced wander.c with laserServer support.
 ...
 *****
 ***** 12/14/2000 1.2
 ***** Major bug fixes on myLaserSweepCallback. It now uses the correct
 ***** bearing and distance to laser targets.  
 *****
 ***** 12/15/2000 1.3
 ***** Added command line option -usePantilt to allow disabling
 ***** of pantiltServer. This is handy if you want to run the program
 ***** in the simulator.
 *****
 ***** 7/16/2001 1.31
 ***** Added 500ms polling of battery voltage for web interface.
 *****
 ***************************************************************************
 ***************************************************************************
 ***************************************************************************/
...
int myLaserSweepCallback ( laserSweepType *data ) {
  ...
  for ( i=0; i!=data->numberLasers; i++ ) {
     if (i < 89) {
/*       laserAngle = -((2 * M_PI) - ((M_PI * (i - 89))/180)); */
       laserAngle = -((M_PI * (89 - i))/180); 
     } 
     if (i > 89) {
/*       laserAngle = (M_PI/2) - ((M_PI * i)/180); */
...
demoPoll(RaiModule * demoModule)
{
  ...
  /* Added by BR 7-16-2001 */
/*  fprintf(stderr, "Requesting Battery voltage.\n"); */
  batteryVoltage();
  /* batteryCurrent(); */
}
...
main(int argc, char** argv )
{
  ...
  params = bParametersAddEntry(params, "", "useIndex", "n");
  ...
  useIndex   = bStrToTruth(bParametersGetParam(params, "", "useIndex"));  
