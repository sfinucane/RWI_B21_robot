 ...
 *****
 ***** wander-colli 
 ***** Enhanced wander-laser.c replacing local collision avoidance 
 ***** with colliServer.
 ***** Brian Rudy (brudy@praecogito.com)
 *****
 ***** 12/30/2000 1.1
 ***** Runs with occasional WD timout, but resumes. Gets stuck in corners.
 ***** Basic 'get bored' functionality. 
 *****
 ***** 12/16/2000 1.0 
 ***** First version to sucessfully compile.
 *****
 ***************************************************************************
 ***************************************************************************
 ***************************************************************************/
...
int indexed = 1;
int sonarsActive = 0;
int boredomFactor = 10; /* if robot hasn't moved +-10cm in 40 tries 
...
ageTargets(void)
{
  ...
  while (next) {
    ...
    if (count == 0) {
      ...
      case TARGET_TYPE_LASER:   countLaser++;   break;
      default: countOther++;
      }
    }
  ...
  if (count == 0) {
    fprintf(stderr,
	    ...
	    "ir[%d] tactile[%d] laser[%d] other[%d]\n",
	    __FILE__, __LINE__, __FUNCTION__, 
	    countTargets, countVirtual, countSonar,
	    countIr, countTactile, countLaser, countOther);
  }
...
/*------------------------------------------------------------*/
/*
 * Determine range and bearing to farthest rear target
 */

struct targetStruct *
farthestRear(float X, float Y, float heading,
	     float *bearing, float *range)
{
  struct targetStruct *next;
  struct targetStruct *target;
  ...
  while (next) {
    ...
    if ( ((nextBearing<-M_PI*.5) && (nextBearing>-M_PI)) || ((nextBearing>M_PI*.5) && (nextBearing<M_PI)) ) {
      if ((target == NULL) || (nextRange > targetRange)) {
	targetRange = nextRange;
	targetBearing = nextBearing;
	target = next;
...
int myLaserSweepCallback ( laserSweepType *data ) {
  ...
  for ( i=0; i!=data->numberLasers; i++ ) {
     if (i < 89) {
       laserAngle = -((M_PI * (89 - i))/180); 
     } 
     if (i > 89) {
       laserAngle = ((M_PI * (i - 89))/180); 
     }
     if (i == 89) {
       laserAngle = 0;
     }
     ...
     /* 
      Data is returned in cm from the array, convert to mm to be
      like data from the sonars.
     */
  
     laserVal = (data->value[i]) * (10 + 5);
  
     if (laserVal/10 < 200) {
        worldAngle = bNormalizeAngle(bWorldAngle(laserAngle, 0.0));
        X = ((float)laserVal)/10 * cos(worldAngle);
        Y = ((float)laserVal)/10 * sin(worldAngle);  

        addTarget(TARGET_TYPE_LASER, 0, ((laserAngle * 180)/M_PI),
                   laserVal, bRobotX(0.0) + X, bRobotY(0.0) + Y,
                   TARGET_LIFE_LASER);
     }   
...
/*      bSetVel(0.0, 0.0); */
...
#if 1
  {
    ...
    float robotAngle;
    int row, col, val;
    float currentTransVel, currentRotVel;   
    ...
    if (target) {
      ...
      case TARGET_TYPE_LASER:   targetType = "L"; break;
      default: targetType = "*?*";
      }
      row = target->row;
      col = target->col;
    ...
    if (count++>40) {
      ...
      currentTransVel = bGetTransVel();
      currentRotVel = bGetRotVel();

      if(((X <= X_old + boredomFactor) && (X_old - boredomFactor <= X)) && ((Y <= Y_old + boredomFactor) && (Y_old - boredomFactor <= Y)) && ((currentTransVel != 0) || (currentRotVel != 0))) {
         if(myi >= 30) {
           fprintf(stderr, "\n Yawn... I am bored, time to go somewhere new.\n");
/*           fartarget = farthestRear(X, Y, heading, &bearing, &range); 
           colliApproachAbsolute(target->X, target->Y, 0,1);
           fprintf(stderr, "Going to X=%8.2f, Y=%8.2f\n\n", fartarget->X, fartarget->Y); */
           fprintf(stderr, "Going to X=%8.2f, Y=%8.2f\n\n", -X, -Y);
           colliApproachAbsolute(-X, -Y, 0, 1);
           myi = 0;
           skipmoves = 20;
         }  
         else {
         ...
         if (skipmoves <= 0) {        
           fprintf(stderr, "Current position X=%8.2f, Y=%8.2f\n", bRobotX(0.0), bRobotY(0.0));
           fprintf(stderr, "New target heading X=%8.2f, Y=%8.2f\n", X, Y);
           colliApproachAbsolute(X, Y, 0, 0);
           X_old = X;
           Y_old = Y;
         }
         else {
...
/*    colliSetVelocity(transVel, rotVel); */
/*    colliApproachAbsolute(X, Y, 0, 1);  */
  }


  if ((ptCount++>5) && usePantilt) {
...
createDemoModule()
{
 ...
 /* colliSetAcceleration(transAccel*10,rotAccel*1023/M_PI); */
 /*  colliSetVelocity(maxSpeed, rotSpeed); */

/* These are defined in colli_modes.ini */
/*   0 DEFAULT_MODE                     */
/*   1 FAST_TRAVEL_MODE                 */
/*   2 FIND_DOOR_MODE                   */
  colliSetMode(0);
...
/*  addPolling(demoModule, demoPoll, 200); */

/* colliServer does this for us */
/*  findRotIndex(); */
  assumeWatchdog();
...
main(int argc, char** argv )
{
 ...
 /* baseConnect(1);   */                  /* hook up to running server*/
  ...
  colliSetVelocity(maxSpeed, rotSpeed);
  colliSetAcceleration(transAccel, rotAccel);
