...
typedef struct {
   ...
   unsigned char poll_wait;	/* is client sleeping in select?  */
...
abMsgToClient (int destNum, ABKMSG *srcPtr) {
   ...
   if ((destNum>0) && (abClient[destNum].poll_wait)) {
     abClient[destNum].poll_wait=0;
...
static void abInterrupt() {
  ...
  if ((abClientNum>0) && (abClient[abClientNum].poll_wait)) {
     abClient[abClientNum].poll_wait=0;
...
static int  abRelease(struct inode* inode,
			 struct file* file)
{
  ABKMSG kmsg;
  int abClientNum;
...
static ssize_t abRead( struct file* file, char* buf, size_t count,loff_t *offset)
{
  ABKMSG *msgPtr;
...
*/
   copy_to_user(&((ABKMSG*)buf)->clientNum,&(msgPtr->clientNum),sizeof(msgPtr->clientNum));   
  }

  /* put message in the user's space */
  msgLen = ((*msgPtr).msg.hdr.msgLen & 0x7F) + 4;
  /*memcpy_tofs(buf, msgPtr, msgLen);*/
  copy_to_user(buf, msgPtr, msgLen);
  return_d(count);
}


static ssize_t abWrite( struct file* file, const char* buf, size_t count,loff_t *offset)
{
  int major;
  int minor;
  int devId;
 ...
 if((count != sizeof(ABMSG)) && (count != sizeof(ABKMSG))) {
  ...
  if ((irq==0) || (port==0) || (abMgrNum==0)) {
	...
	printk("irq %d port %d abMgrNum %d", irq,port,abMgrNum);
    return_d(-EUNATCH);
  }
...
*/
  copy_from_user(&(writeMsg.clientNum),&((ABKMSG*)buf)->clientNum,sizeof(writeMsg.clientNum));
...
#endif
  copy_from_user(&writeMsg, buf, 4); /* get the header */
  ...
  if (msgLen)			/* get the rest */
    copy_from_user((unsigned char *)(&writeMsg)+4, buf+4, msgLen);
...
*/
   	copy_from_user(&newIrq,dataPtr++,sizeof(unsigned long));
	copy_from_user(&newPort,dataPtr++,sizeof(unsigned long));
...
*/
    copy_from_user(&oldIrq,dataPtr++,sizeof(unsigned long));
    copy_from_user(&oldPort,dataPtr++,sizeof(unsigned long));
...
*/
    ...
    copy_from_user(&clientNum,dataPtr++,sizeof(unsigned long));   
...
*/
  ...
  copy_from_user(&devNum,dataPtr++,sizeof(unsigned long));
    copy_from_user(&clientNum,dataPtr++,sizeof(unsigned long));
...
*/
   ...
   copy_from_user(&devNum,dataPtr++,sizeof(unsigned long));
    copy_from_user(&devStatus,dataPtr++,sizeof(unsigned long));
...
/*
static unsigned int abSelect(struct file *file, int sel_type, poll_table *wait) 
*/
static unsigned int abSelect(struct file *file,  poll_table  *wait) {
  int abClientNum;
  unsigned long flags;
  unsigned int mask;
  pgmTrace();
...
*/
  ...
  /* printk("Devuelvo %x en select\n",mask);*/
    return_d(mask);
  }

  abClient[abClientNum].poll_wait = 1;
  poll_wait(file,&abClient[abClientNum].queue, wait);
  sti();
/*
  restore_flags(flags);
*/
/*printk("Devuelvo %d (0) en select\n",mask);*/
  return_d(mask);
}
