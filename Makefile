...
PACKAGES = 	abus sys scheduler utils mcp msp consoleLib tcx \
		RobotServer baseServer speechServer armServer \
		sound libgd network \
		ezx robot obstacleServer robotDetect localize colli \
		buttons plan controller \
		map sonarint Drawing10 simulator2 \
		pantilt vision speech \
		laserint router collgraph cameraServer beeExamples \
		laserServer laserServer2 convert bee-light
