/*
 * $Id: grab.c,v 1.1 2002/09/14 16:37:46 rstone Exp $
 *
 */
#define SEBASTIAN2
...
void gotframe( int signalnumber ) {
  ...
  struct timeval TCX_waiting_time = {0, 0};
  static struct timeval last_saving_time[2] = {{0, 0}, {0, 0}};
  ...
  if (last_grabber == numGrabber){
    ...
    c  = METEOR_CAP_CONTINOUS; 
    if (ioctl(meteor[1-numGrabber], METEORCAPTUR, &c)) 
      fprintf( stderr,"ioctl CaptContinuous failed\n");
    */
  }
  last_grabber = numGrabber;
#endif


  fprintf(stderr, "%d", numGrabber);
  ...
  if ( saving[numGrabber] ) {
    ...
    time_difference = 
      ((float) (this_time.tv_sec - last_saving_time[numGrabber].tv_sec))
      + (((float) (this_time.tv_usec - last_saving_time[numGrabber].tv_usec))
	 /  1000000.0);
    /*fprintf(stderr, "?");    */
    if (time_difference >= savingdelay){
      fprintf(stderr, "#");
      last_saving_time[numGrabber].tv_sec  = this_time.tv_sec;
      last_saving_time[numGrabber].tv_usec = this_time.tv_usec;
...
#ifdef SEBASTIAN2
      memcpy(location_shmem[numGrabber], &activeStatusReport, 
	     (size_t) sizeof(statusReportType));
      {
	int i;
	unsigned char *p = (unsigned char *) (&activeStatusReport);
	for (i = 0; i < sizeof(statusReportType); i++)
	  fprintf(stderr, " %.2x", *p++);
	fprintf(stderr, "\n");
      }
      if ( ROWS*COLS*4 != fwrite( location_shmem[numGrabber],
				  sizeof(char),
				  ROWS*COLS*4, videofp[numGrabber] ) ) {
#else
 ...
 * *** empty log message ***
 *
 * Revision 1.37  1998/09/18 16:04:17  thrun
 * .
 *
 * Revision 1.36  1998/08/16 19:50:01  thrun
 * can now genrate arbitrary sub-images and downsample them.
 *
 * Revision 1.35  1998/08/08 22:19:56  thrun
 * -rate as commandline parameter: specifies frequency for
 * saving images on file.
 *
 * Revision 1.34  1998/08/08 21:25:30  thrun
 * modifictions needed to run the cameraServer on the new robot.
 *
 * Revision 1.33  1998/08/08 18:44:43  thrun
