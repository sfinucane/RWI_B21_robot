/*
 * $Id: grab.h,v 1.1 2002/09/14 16:37:46 rstone Exp $
 *
 */
#ifndef GRAB_H_LOADED
#define GRAB_H_LOADED

/*  Here are 2 options you can give or not on the compile line: */
/*   -DHAVE_MATROX_METEOR */
/*   -DHAVE_COLOR_QUICKCAM */

#define MAX_INPUTS_PER_GRABBER 2
...
extern char *tcxMachine;	/* which tcxhost do we use? */

extern int useGrabber;		/* which grabber do we use? */
...
extern unsigned char *location_shmem[MAX_INPUTS_PER_GRABBER];

/* id of the shared memory segment */
extern int shmid[MAX_INPUTS_PER_GRABBER];

/* this always points to the latest camera image from each grabber, in
   the format RGBxRGBx, of size ROWS x COLS x 4 bytes.  The "x" byte
   is padding. */
extern unsigned char *pCameraImage[MAX_INPUTS_PER_GRABBER]; /* camera images */
                             
/* frames to save (set by TCX message) */
extern int saveframes[MAX_INPUTS_PER_GRABBER];
                             
/* flag that will be set by a TCX message */
extern int saving[MAX_INPUTS_PER_GRABBER];
                             
/* the filehandle for saving frames */
extern FILE *videofp[MAX_INPUTS_PER_GRABBER];
                             
/* state of the file */
extern int savefileopen[MAX_INPUTS_PER_GRABBER];
                             
/* the name of the file, where we save the frames */
extern char filename[MAX_INPUTS_PER_GRABBER][128];
...
int startGrabber( int numGrabber );
int stopGrabber( int numGrabber );
...
/* *** empty log message ***
/*
 * Revision 1.18  1999/04/21 22:25:01  fox
