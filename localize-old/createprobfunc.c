			 ...
			 int delta, float factor)
{
  ...
  for (x = 0; x < numberOfExpectedDistances; x++){
    ...
    for ( y = 0; y < x; y++)
      tab->prob[x][y] =
	fMax(params.minProbShort, factor * gauss( y, sigma, x));
    for ( y = x; y < numberOfMeasuredDistances; y++)
      tab->prob[x][y] =
	fMax(params.minProbFar, factor * gauss( y, sigma, x));
    }
  for (x = 0; x < numberOfExpectedDistances; x++){
  ...
  for (x = 0; x < numberOfExpectedDistances; x++)
    tab->prob[x][numberOfMeasuredDistances - 1] = params.maxRangeMinLevel +
      maxRangeValue( params, *tab, x);
  /*
...
void
dumpDistProbFunction(char *fileName, distProbTable probTab)
{
  int x, y;
	       ...
	       distProbTable tab,  distProbTable estimated)
{
  int x;
	...
	fprintf(fp, "%d %f %f\n",
		x,
		(float) tab.prob[row][x], estimated.prob[row][x]);
     ...
     char *argv[];
{
  ...
  if (argc != 9){
    fprintf(stderr, "usage: %s datafile sigmafrom sigmato minprobclose minprobfar maxfrom maxto factor\n", argv[cnt]);
    exit(1);
  }
  ...
  cnt++;
  params.sigmaShort = atof(argv[cnt]);
  if (params.sigmaShort <= 0){
    fprintf(stderr, "invalid sigmafrom: %f\n", params.sigmaShort);
    exit(-1);
  }

  cnt++;
  params.sigmaFar = atof(argv[cnt]);
  if (params.sigmaFar <= 0){
    fprintf(stderr, "invalid sigmato: %f\n", params.sigmaFar);
    exit(-1);
  }

  cnt++;
  params.minProbShort =  atof(argv[cnt]);
  if (params.minProbShort <= 0){
    fprintf(stderr, "invalid minprobclose: %f\n", params.minProbShort);
    exit(-1);
  }

  cnt++;
  params.minProbFar =  atof(argv[cnt]);
  if (params.minProbFar <= 0){
    fprintf(stderr, "invalid minprobfar: %f\n", params.minProbFar);
    exit(-1);
  }

  params.maxRange.numberOfLins = 1;
  params.maxRangeMinLevel = 0.0;
  
  cnt++;
  params.maxRange.probShort[0] = atof(argv[cnt]);
  if (params.maxRange.probShort[0] <= 0){
    fprintf(stderr, "invalid maxfrom: %f\n", params.maxRange.probShort[0]);
    exit(-1);
  }

  cnt++;
  params.maxRange.probFar[0] = atof(argv[cnt]);
  if (params.maxRange.probFar[0] <= 0){
    fprintf(stderr, "invalid maxto: %f\n", params.maxRange.probFar[0]);
    exit(-1);
  }

  cnt++;
  factor = atof(argv[cnt]);
  if (factor <= 0 || factor > 100){
    fprintf(stderr, "invalid factor: %f\n", factor);
    exit(-1);
  }

  readSampleData( fileName, resolution, &probTab);
  ...
  computeDistProbFunction( params, &estimatedProbTab,
			   probTab.numberOfExpectedDistances,
			   probTab.numberOfExpectedDistances,
			   maxRangeDelta, factor);

  
  dumpDistProbFunction("estimated.gnu",estimatedProbTab);
  if (0) {
    int x;
    char file[30];
    ...
    for (x = 0; x<probTab.numberOfExpectedDistances; x++){
      ...
      printSingleRow(file, x, probTab, estimatedProbTab);
    }
  }
  dumpDistProbFunction("measured.gnu",probTab);
  ...
  dumpDistProbFunction("normalized.gnu",estimatedProbTab);
