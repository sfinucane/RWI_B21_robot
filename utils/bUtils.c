 ...
 *****
 *****                  v1.2   (released September 9, 1997)
 *****
		    ...
		    const char *name)
{
  ...
  if ((!list) || (!prefix) || (!name)) {
    /* The following will lead to a segmentation fault
       if the parameter we are looking fir is NOT in the
       list.
    return(NULL);
    */

    fprintf(stderr,"\n\n\n !!!   W A R N I N G   !!!");
    fprintf(stderr,"\n !!!   The paramter >%s %s< is not in beeSoft.ini",
	    prefix, name);
    fprintf(stderr,"\n !!!   The paramter will be set to the value 4711 :)\n\n\n");
    return(berror);
  }
...
bParametersFillParams(struct bParamList *list)
{
   ...
   * basic robot system
   */
  test = bParametersGetParam(list, "robot", "pioneer");

  if ((test == NULL) || (strcasecmp(test, "no") == 0)) {
    fprintf(stderr,"\n\n\n IT IS A B21");
  ...
  if ((prefix != NULL) && (strcasecmp(prefix, "none") != 0)) {
  ...
  prefix = bParametersGetParam(list, "robot", "laser_front");

  if ((prefix != NULL) && (strcasecmp(prefix, "none") != 0)) {
    bRobot.laser_front_type = bParametersGetParam(list, prefix, "laser_front.type");
    bRobot.laser_front_host = bParametersGetParam(list, prefix, "laser_front.host");
    bRobot.laser_front_dev  = bParametersGetParam(list, prefix, "laser_front.dev");
    
    bRobot.laser_front_bps  = 
      bStrToBaud(bParametersGetParam(list, prefix, "laser_front.bps"));
  }

  
  prefix = bParametersGetParam(list, "robot", "laser_rear");

  if ((prefix != NULL) && (strcasecmp(prefix, "none") != 0)) {
    bRobot.laser_rear_type  = bParametersGetParam(list, prefix, "laser_rear.type");
    bRobot.laser_rear_host  = bParametersGetParam(list, prefix, "laser_rear.host");
    bRobot.laser_rear_dev   = bParametersGetParam(list, prefix, "laser_rear.dev");
    
    bRobot.laser_rear_bps   = 
      bStrToBaud(bParametersGetParam(list, prefix, "laser_rear.bps"));
  }
  ...
  if ((test != NULL) && (strcasecmp(test, "yes")==0)) {
    fprintf(stderr,"\n\n\n IT IS A PIONEER");
    prefix = bParametersGetParam(list, "robot", "name");

    bRobot.pio_type             = bParametersGetParam(list, prefix, "pio.type");
    bRobot.pio_host             = bParametersGetParam(list, prefix, "pio.host");
    bRobot.pio_dev              = bParametersGetParam(list, prefix, "pio.dev");
    
    bRobot.pio_bps              = bStrToBaud(bParametersGetParam(list, prefix, "pio.bps"));

    bRobot.pio_radius           = atof(bParametersGetParam(list, prefix, "pio.radius"));
    bRobot.pio_cntperCm         = atof(bParametersGetParam(list, prefix, "pio.cntperCm"));
    bRobot.pio_defcntperdeg     = atof(bParametersGetParam(list, prefix, "pio.defcntperdeg"));
    bRobot.pio_defdriftdegperCm = atof(bParametersGetParam(list, prefix, "pio.defdriftdegperCm"));
    bRobot.pio_tranvelfac       = atof(bParametersGetParam(list, prefix, "pio.tranvelfac"));
    bRobot.pio_rotvelfac        = atof(bParametersGetParam(list, prefix, "pio.rotvelfac"));
    bRobot.pio_velconvfac       = atof(bParametersGetParam(list, prefix, "pio.velconvfac"));
    bRobot.pio_statvelfac       = atof(bParametersGetParam(list, prefix, "pio.statvelfac"));
    bRobot.pio_maxvel           = atof(bParametersGetParam(list, prefix, "pio.maxvel"));
    bRobot.pio_maxorient        = atof(bParametersGetParam(list, prefix, "pio.maxorient"));
    bRobot.pio_max_xy           = atof(bParametersGetParam(list, prefix, "pio.max_xy"));
    bRobot.pio_posoffset        = atof(bParametersGetParam(list, prefix, "pio.posoffset"));
    bRobot.pio_max_tranvel      = atof(bParametersGetParam(list, prefix, "pio.max_tranvel"));
    bRobot.pio_max_rotvel       = atof(bParametersGetParam(list, prefix, "pio.max_rotvel"));


    string = bParametersGetParam(list, prefix, "pio.shape");
    ii = 0;
    while (index[0] = ii, (element = bGetArrayElementM(string, 1, index))) {
      bRobot.pio_shape[ii] = atof(element);
      ii++;
      // fprintf(stderr,"\n--->%f<--->%s<--->%s<---",atof(element),string,prefix);
      free(element);
    }

    /* sonar */
    bRobot.pio_sonar_number   = atoi(bParametersGetParam(list, prefix, "pio_sonar.number"));
    

    
    string = bParametersGetParam(list, prefix, "pio_sonar.offsetforward");
    ii = 0;
    while (index[0] = ii, (element = bGetArrayElementM(string, 1, index))) {
      bRobot.pio_sonar_offsetforward[ii] = atof(element);
      ii++;
      // fprintf(stderr,"\n--->%f<--->%s<--->%s<---",atof(element),string,prefix);
      free(element);
    }
    ...
    string = bParametersGetParam(list, prefix, "pio_sonar.offsetsideward");
    ii = 0;
    while (index[0] = ii, (element = bGetArrayElementM(string, 1, index))) {
      bRobot.pio_sonar_offsetsideward[ii] = atof(element);
      ii++;
      // fprintf(stderr,"\n--->%f<--->%s<--->%s<---",atof(element),string,prefix);
      free(element);
    }
    ...
    string = bParametersGetParam(list, prefix, "pio_sonar.sonarangles");
    ii = 0;
    while (index[0] = ii, (element = bGetArrayElementM(string, 1, index))) {
      bRobot.pio_sonar_sonarangles[ii] = atof(element);
      ii++;
      // fprintf(stderr,"\n--->%f<--->%s<--->%s<---",atof(element),string,prefix);
      free(element);
    }
...
#ifdef FGAN
    bRobot.pio_ir = atoi(bParametersGetParam(list, prefix, "pio_ir.number"));
#endif
    
    
    /* power */
    string = bParametersGetParam(list, prefix, "pio_volt.warn");
    bRobot.pio_volt_warn  = 0.0;
    if (string) {
      bRobot.pio_volt_warn  = atof(string);
    }
    string = bParametersGetParam(list, prefix, "pio_volt.max");
    bRobot.pio_volt_max  = 0.0;
    if (string) {
      bRobot.pio_volt_max  = atof(string);
    }
    string = bParametersGetParam(list, prefix, "pio_volt.panic");
    bRobot.pio_volt_panic  = 0.0;
    if (string) {
      bRobot.pio_volt_panic  = atof(string);
    }

    /* laser */
    bRobot.pio_laser_type = bParametersGetParam(list, prefix, "pio_laser.type");
    bRobot.pio_laser_host = bParametersGetParam(list, prefix, "pio_laser.host");
    bRobot.pio_laser_dev  = bParametersGetParam(list, prefix, "pio_laser.dev");
    bRobot.pio_laser_bps  = bStrToBaud(bParametersGetParam(list, prefix, "pio_laser.bps"));
  }
