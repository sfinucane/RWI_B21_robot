...
/*wasi*/
extern int useSSP;
extern int useNbrInner;
extern int useNbrOuter;
/******/
...
/*wasi*/
    ...
    else if (!strcmp(argv[i], "-nis"))
    {
       useNbrInner = TRUE;
       printf("Using neighbours of inner sensors\n");
    }    
    else if (!strcmp(argv[i], "-nos"))
    {
       useNbrOuter = TRUE;
       printf("Using neighbours of outer sensors\n");
    }    
    else
      bug = 1;
  }
  ...
  if (bug == 1) {
    fprintf(stderr, "\n--Usage: '%s [-nodisplay] [-tcx] [-file <filename>] [-delay <sec>] [-nolog] [-pioneer] [-pioneerf] [-pioneer2] [-pioneerat] [-robot <name>] [-dev <device_path>] [-nis] [-nos]\n", argv[0]);
    exit(1);
  }
  else if (bug == 2) {
		  ...
		  ALL_PTR                  all)
{
  ...
  if (robot_specifications != NULL){
    ...
    /*wasi*/
    else if ((bRobot.sonar_cols[0] == 8)||(bRobot.sonar_cols[0] == 7)){
      robot_specifications->first_sensor_angle		   = -90.0;
    }
    /***** */
...
void allocate_everything(ROBOT_STATE_PTR    robot_state,
			 ...
			 ROBOT_SPECIFICATIONS_PTR robot_specifications)
{
  int i= 0, global_size= 0, local_size= 0;
  ...
  if (!program_state->maps_allocated){
    ...
    if (!program_state->maps_allocated){
      local_map     = (float *) (calloc(local_size,sizeof(float)));
      local_robot   = (int *)   (calloc(local_size,sizeof(int)));
      local_active  = (int *)   (calloc(local_size,sizeof(int)));
    ...
    robot_specifications->sensor_angles = 
      (float *) (calloc(robot_specifications->num_sensors,sizeof(float)));
	...
	* 0.5 * (sin(((float) i) * 0.3) + 1.0); 
    ...
    if (useSSP) {
      int j = 0;
      /*
      robot_specifications->sensor_angles[0] = -90.0;
      robot_specifications->sensor_angles[1] = -30.0;	
      robot_specifications->sensor_angles[2] = -15.0;
      robot_specifications->sensor_angles[3] =   0.0;
      robot_specifications->sensor_angles[4] =  15.0;	
      robot_specifications->sensor_angles[5] =  30.0;
      robot_specifications->sensor_angles[6] =  90.0;
      robot_specifications->sensor_angles[7] = 180.0;
      */
      j = robot_specifications->num_sensors-1;
      
      for (i = 0; i < robot_specifications->num_sensors; i++ ) {
	robot_specifications->sensor_angles[i] = bRobot.pio_sonar_sonarangles[j];
	// fprintf(stderr,"\n%d - rs:%f <>%d - br:%f",i,robot_specifications->sensor_angles[i],j,bRobot.pio_sonar_sonarangles[j]);
	j--;
      }
      fprintf(stderr,"\n");
    }    
...
#if DEBUG_SHIT
    for (i = 0; i<4;i++)
      fprintf(stderr,"\n%d--->%f",i,bRobot.pio_shape[i]);
    
    fprintf(stderr,"\n--------\n");
    
    for (i = 0; i<bRobot.pio_sonar_number;i++)
      fprintf(stderr,"\n%d--->%f",i,bRobot.pio_sonar_offsetforward[i]);
    
    fprintf(stderr,"\n--------\n");
    
    for (i = 0; i<bRobot.pio_sonar_number;i++)
      fprintf(stderr,"\n%d--->%f",i,bRobot.pio_sonar_offsetsideward[i]);
    
    fprintf(stderr,"\n--------\n"); 
    
    for (i = 0; i<bRobot.pio_sonar_number;i++)
      fprintf(stderr,"\n%d--->br:%f <> rs:%f",i,bRobot.pio_sonar_sonarangles[i],robot_specifications->sensor_angles[i]);
...
/******/
    ...
    for (i = 0; i < local_size; i++)
      local_map[i]     = 0.5;
    // The next two are useless. We use calloc
    /*    memset(local_active, 0, local_size*sizeof(int));
	  memset(local_robot, 0, local_size*sizeof(int)); */
...
void clear_all_maps(ROBOT_STATE_PTR    robot_state,
		    ...
		    ROBOT_SPECIFICATIONS_PTR robot_specifications)
{
  int x= 0, sizee= 0, index= 0;

  sizee = robot_specifications->local_map_dim_x*robot_specifications->local_map_dim_y * sizeof(int);
  
  if (local_active != NULL)
    memset(local_active, 0, sizee);
  if (local_robot != NULL)
    memset(local_robot, 0, sizee);
  
  sizee = robot_specifications->local_map_dim_x*robot_specifications->local_map_dim_y;
