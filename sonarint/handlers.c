				      ...
				      BASE_update_status_reply_ptr status)
{
  ...
  float diff_x=0.0, diff_y=0.0, distance=0.0, angle=0.0, new_x=0.0, new_y=0.0;
  float status_new_x=0.0, status_new_y=0.0;
  char out_text[256];
  struct timeval t; 
  time_t current_time;
...
static float last_sonar_x=0.0;
static float last_sonar_y=0.0;
static float last_sonar_orientation=0.0;
  ...
  if (!program_state->processing_script || ref == NULL){
    ...
    if (last_message_time.tv_sec != 0)
      printf("Sonar reading took %d msec\n", (int)
		 ...
		 last_message_time.tv_usec) * 0.001));
	...
	for (i = 0, j = 0; i < robot_specifications->num_sensors; i++, j--){
      ...
      else { /* !!! look here Frank !!! */
	
	for (i = 0, j = 6; i < robot_specifications->num_sensors; i++, j--){
	  if (j < 0)
	    j += robot_specifications->num_sensors;
	  if (sonar->values[i] < 0.0)
	    robot_state->sensor_values[j] = robot_specifications->robot_size;
...
/******/
      ...
      compute_local_map(neural_network, robot_specifications, program_state,robot_state);
      /*
...
{  int i=0, j=0;
  ...
  tmp = robot_specifications->num_sensors /2;
  for (i = 0; i < 360; i++){
    j = i % tmp;
    if (i < tmp) {
      (laser->f_reading != NULL)
				...
				ROBOT_STATE_PTR          robot_state)
{
  ...
  local.char_likelihoods   = (unsigned char *)
    calloc((local.size_x * local.size_y),sizeof(float));

  tmp = local.size_x * local.size_y;
